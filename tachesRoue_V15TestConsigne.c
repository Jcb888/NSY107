...
rev 15.1 test montee roue active ligne 304
instrumentation des tps desactive a part stats à la fin
***************************************************************/
	...
	if (rt_queue_inquire(&idFileEchantillonsCourbes, &queueInfo)!=0)
		rt_printf("erreur interrogation file");
				...
				for (i = 0; i < queueInfo.nmessages; i++){
					...
					rt_queue_read(&idFileEchantillonsCourbes, &echantillon, sizeof(echantillon), TM_NONBLOCK);//retour = nbBytes lus
...
void tDureePeriodesEtInfosCourbes(){//alias T1
	...
	for (;;){
		...
		DisplayInfoTasks();//on peut le garder car ne s'active qu'en sortant
	}//fin for(;;)
...
/****************************************/
	for (;;){// boucle infini pour que la tache existe
		// sem_p destine a l'initialisation lors du premier passage ou initialisation nouvelle phase)
		...
		while (termine == 0){
			...
			// pour test de la fonction a activer seule.

			if (cde < 10.0){
				applicationIConsigne(++cde);
				rt_printf("applicationIConsigne(%f)\n", cde);
			}
			...
			rt_printf("valeur switch loi : %d \n", parametresExperimentation.loi);
				...
				case 0 :{//Boucle ouverte
					...
					if (parametresExperimentation.profilConsigne == 0){//echelon
					...
					if (parametresExperimentation.profilConsigne == 1){// rampe
				...
				case 11 :{ //Boucle position voir cahier des charges page 8 ?? consigne en Radian --> ?? conversion en amperes ??
