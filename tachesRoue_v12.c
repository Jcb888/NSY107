...
//
//  rev 12
//	Fait ajout Mutex, debut codage profil consigne
//  ToDo  Mutex 
profil consigne , option loi
//	
//***************************************************************/
...
/**************************************/
	...
	float tabCapteurs[4];
	rt_mutex_acquire(&idMutexCAN, TM_INFINITE);
...
/***************************************************************/
	...
	tableauReels[2] = tableauReels[2] * 1000.0;// adaptation format temps e, ms
...
void tGererLoiConsigne(){// allias T3
	...
	for (;;){// boucle infini pour que la tache existe
		...
		rt_printf("code retour rt_sem_p: %d \n", retour);
		//pour gestion profil montee
		int nbBouclesIncrement = (parametresExperimentation.dureeMonteeConsigne / parametresExperimentation.periodeActivationLoi);// en ms
		// ex nbBouclesIncrement = xx si 
		int valeurIncrement = 
		...
		while (termine == 0){
			...
			float cde = 0.0;
			TypeEchantillon echantillon;
			// Pas trouve de gestion du mutex dans les SP alors je le mets ici
			rt_mutex_acquire(&idMutexCAN, TM_INFINITE);
			acquisitionEchantillon(&echantillon);
			rt_mutex_release(&idMutexCAN);
