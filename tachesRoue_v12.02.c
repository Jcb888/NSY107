...
//	tachesRoue.c												*)
//
//rev 12.02 repris depuis v11 sur alim hs
...
void envoyerDernierBlocEchantillons(){
	...
	if (rt_queue_inquire(&idFileEchantillonsCourbes, &queueInfo) != 0)
		rt_printf("erreur interrogation file");
				...
				//si queueInfo.nmessages = 0 on envoit un tableau vide, cas normalement non possible, generer erreur ?
...
/***************************************************************/
	...
	for (i = 0; i < 15; i++){
		...
		rt_printf("parametres experimentation.coeffLoi[i]: %d valeur: %f \n ", i, parametresExperimentation.coeffLoi[i]);
	}
